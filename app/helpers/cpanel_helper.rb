module CpanelHelper
  ...
  def generate_creating_history(saved_new_object)
    saved_new_object.management_histories.create(user: current_user, action_type: :creating)
  end

  class ActiveRecord::Base
    def assign_attributes_with_history(new_attributes, by_admin)
      assign_attributes new_attributes
      if valid?
        changes.each do |k, v|
          management_histories.create(user: by_admin, action_type: :updating,
                                      field_changed: k, old_value: v[0], new_value: v[1])
        end
      end
    end
